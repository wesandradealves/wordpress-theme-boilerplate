...
class SeoRevisions extends CommonSeoRevisions\SeoRevisions {
	...
	public function getVueDataCompare() {
		...
		$itemToId = ! empty( $_GET['to'] ) ? absint( $_GET['to'] ) : 0;
		...
		$itemFromId = ! empty( $_GET['from'] ) ? absint( $_GET['from'] ) : 0;
		if ( $itemFromId ) {
	...
	private function setEligibleFields() {
		// Fields with no title serve to compare data later and make sure the column changed (and they have "special" columns under the comparison page).
		$this->eligibleFields = [
