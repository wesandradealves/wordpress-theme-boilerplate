...
class ObjectRevisions {
	...
	public function getFieldsDiff( $fromId, $toId ) {
		...
		foreach ( aioseo()->seoRevisions->eligibleFields as $key => $label ) {
			...
			$contentFrom = $revisionFrom->exists() ? aioseo()->seoRevisions->helpers->prepareRevisionDataFieldValue( $key, $revisionFromData[ $key ], $revisionFrom ) : '';
