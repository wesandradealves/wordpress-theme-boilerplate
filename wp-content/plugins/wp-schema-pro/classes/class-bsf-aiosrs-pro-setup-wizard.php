...
if ( ! class_exists( 'BSF_AIOSRS_Pro_Setup_Wizard' ) ) :
	...
	class BSF_AIOSRS_Pro_Setup_Wizard {
		...
		public function setup_wizard() {
			...
			if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'aiosrs-pro-setup-wizard' ) && ! empty( $_POST['save_step'] ) && isset( $this->steps[ $this->step ]['handler'] ) ) {
		...
		public function setup_wizard_header() {
			...
			<body class="aiosrs-pro-setup-wizard wp-core-ui">
				<div id="aiosrs-pro-logo">
					...
					<?php } else { ?>
						<a href="https://wpschema.com/" target="_blank"><img src="<?php echo esc_url( BSF_AIOSRS_PRO_URI . '/admin/assets/images/schema-pro.png' ); ?>" alt="<?php esc_html_e( 'Schema Pro', 'wp-schema-pro' ); ?>" ></a>
				<?php } ?>
				</div>
			<?php
		}
		...
		public function global_schemas_save() {
			...
			if ( isset( $_POST['wp-schema-pro-global-schemas'] ) ) {
				...
				if ( isset( $new_data['enable_bread'] ) ) {
					$old_data                 = $_POST['wp-schema-pro-global-schemas']['breadcrumb'];
